...
void Shield::create_all_shield_components() {
    ...
    for (int i = 1; i <= ceil(1.0 * constants::NUM_SHIELD_COMPONENTS / 2); i++) {
        heights.push_back(2 * i);
    }
    for (int i = ceil(1.0 * constants::NUM_SHIELD_COMPONENTS / 2) + 1; i < constants::NUM_SHIELD_COMPONENTS; i++) {
        heights.push_back(heights[constants::NUM_SHIELD_COMPONENTS - i - 1]);
    }
    for (int i = 0; i < constants::NUM_SHIELD_COMPONENTS; i++) {
